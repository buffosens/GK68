...
function onSingleSelect(data) {
  for (var i = 0; i < DEVICE.profile.KeySet.length; i++) {
    if(DEVICE.profile.KeySet[i].Index === data.LogicCode) {
      ...
      if(!$("#tool_singlebutton").is(':hidden')){
        $("#com_key").focus();
      }
      break;
    }
...
function renderLamp() {
  ...
  for (var i = 0; i < CMS.les.length; i++) {
    ...
    if(CMS.deviceConfig.LeCate && CMS.les[i].LeCate && (CMS.deviceConfig.LeCate == CMS.les[i].LeCate)) {
      str += '<li class="menu-item menu-item-light menu-item-file" data-index="' + i + '" data-type="' + 0 + '" data-guid="' + CMS.les[i].GUID + '">\
      <span>' + CMS.les[i].Name + '</span>' +combicostr + '</li>';
    } else if (CMS.les[i].LeType == "combination") {
      strcomb += '<li class="menu-item menu-item-light menu-item-file" data-index="' + i + '" data-type="' + 0 + '" data-guid="' + CMS.les[i].GUID + '">\
      <span>' + CMS.les[i].Name + '</span>' +combicostr + '</li>';
    } else if (CMS.deviceConfig.LeCate && (CMS.les[i].LeType == "combination")) {
      strcomb += '<li class="menu-item menu-item-light menu-item-file" data-index="' + i + '" data-type="' + 0 + '" data-guid="' + CMS.les[i].GUID + '">\
      <span>' + CMS.les[i].Name + '</span>' +combicostr + '</li>';
    } else if (!CMS.deviceConfig.LeCate && !CMS.les[i].LeCate) {
      str += '<li class="menu-item menu-item-light menu-item-file" data-index="' + i + '" data-type="' + 0 + '" data-guid="' + CMS.les[i].GUID + '">\
  ...
  $("#tool_lamp").find(".menu-list:eq(0)").find("ul").append(str);
  $("#tool_lamp").find(".menu-list:eq(0)").find("ul").append(strcomb);
}

function lampfunc() {
  $("#tool_lamp").find(".func-setting").find("ul").find(".menu-item").unbind('click').click(function() {
    $("#tool_lamp .menu-item-light.selected").removeClass("selected");
...
function checkDriverLE() {
  ...
  for (var i = 0; i < DEVICE.profiles[1].DriverLE.length; i++) {
    ...
    var flag = true;
    for (var j = 0; j < CMS.les.length; j++) {
      if (DEVICE.profiles[1].DriverLE[i].GUID == CMS.les[j].GUID) {
        if (DEVICE.profiles[1].DriverLE[i].Name != CMS.les[j].Name) {
          DEVICE.profiles[1].DriverLE[i].Name = CMS.les[j].Name;
        }
        flag = false;
        break;
      }
