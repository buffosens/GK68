...
function renderMacroList(data, title) {
  if (title) {
    ...
    var macroTitle = "<span cms-lang='macro_list'>"+$.multilang("macro_list")+"</span>" + "<i>&nbsp;&nbsp;/&nbsp;&nbsp;</i><span class='active'>" + title + "</span>";;
...
function macroOperate() {
  $("#macro_items").find("li").off('click').click(function() {
    if($(this).data("type") == 1) {
      ...
      currentdirindex = $(this).index()+1;
      renderMacroList(Macros.macros[currentdirindex-1].Data, name);
    } else {
      var name = $(this).find("p").text();
      var guid = $(this).data("guid");
      ...
      $(this).find("img").css({"boxShadow": "0px 0px 0px 2px #ff0000"});
...
function makeMacro() {
  ...
  if(Macros.macroData.TaskList.length > 0) {
    window.parent.confirm($.multilang("macro_record_warning_title"),$.multilang("macro_record_warning_info"),function(e) {
      if(e) {
...
function initInsertWindow() {
  ...
  $("#macro_item_record_start").is(":hidden") && $("#macro_item_record_start").show();
  $("#macro_item_record_stop").hide();
}
...
function addMacroFile() {
  ...
  window.parent.prompt($.multilang("macro_creat_macro"), $.multilang("macro_input_name"), name, function(result, value) {
    ...
    if(currentdirindex === 0) {
      ...
      Macros.macros[currentdirindex-1].Data.push(macroItem);
    }
    //Macros.macros.push(macroItem);
    window.parent.writeMacroList(Macros.macros, function() {  
      console.log('写宏列表成功');
      ...
      if(currentdirindex === 0) {
        ...
        renderMacroList(Macros.macros[currentdirindex-1].Data,Macros.macros[currentdirindex-1].Name);
      }
...
function deleteMacroFile() {
  ...
  stopRecordMacro();
  window.parent.confirm($.multilang("macro_delete_warning_title"),$.multilang("macro_delete_warning_info"),function(e) {
    if(e) {
...
function exportMacroFile() {
  if(Macros.macroData) {
    window.parent.exportMacro(Macros.macroData,function(data) {
      window.parent.success($.multilang('macro_export_success'));
    });
  }else{
    window.parent.warning($.multilang('macro_selecte_macro'));
  }
...
function importMacroFile() {
  window.parent.importMacro(function (data) {
    var newMacroData = JSON.parse(data);
    var flag = true;
    if(currentdirindex === 0) {
      for(var j = 0; j < Macros.macros.length; j++) {
        //配置文件已存在
        if(Macros.macros[j].Type === 0) {
          if(Macros.macros[j].GUID === newMacroData.GUID) {
            flag = false;
            Macros.macros[j].MacroName = newMacroData.Name;
            window.parent.confirm($.multilang("macro_prompt"), $.multilang("macro_file_exist"), function() {
              window.parent.writeMacroFile(newMacroData.GUID, newMacroData, function(data) {
                Macros.macros[j].Name = newMacroData.MacroName;
                window.parent.writeMacroList(Macros.macros,function() {
                  renderMacroList(Macros.macros);
                });
              });
      ...
      for(var j = 0; j < Macros.macros[currentdirindex-1].Data.length; j++) {
        //分类目录下配置文件已存在
        if(Macros.macros[currentdirindex-1].Data[j].Type === 0) {
          if(Macros.macros[currentdirindex-1].Data[j].GUID === newMacroData.GUID) {
            flag = false;
            Macros.macros[currentdirindex-1].Data[j].MacroName = newMacroData.Name;
            window.parent.confirm($.multilang("macro_prompt"), $.multilang("macro_file_exist"), function() {
              window.parent.writeMacroFile(newMacroData.GUID, newMacroData, function(data) {
                Macros.macros[currentdirindex-1].Data[j].Name = newMacroData.MacroName;
                window.parent.writeMacroList(Macros.macros,function() {
                  renderMacroList(Macros.macros[currentdirindex-1].Data,Macros.macros[currentdirindex-1].Name);
                });
              });
    ...
    if(flag) {
      window.parent.writeMacroFile(newMacroData.GUID, newMacroData, function(data) {
        var newLeItem = {
          "GUID": newMacroData.GUID,
          "Name": newMacroData.MacroName,
          "Icon": "default.png",
          "Type": 0
        }
        if(currentdirindex === 0) {
          Macros.macros.push(newLeItem);
        }else{
          Macros.macros[currentdirindex-1].Data.push(newLeItem);
        }
        window.parent.writeMacroList(Macros.macros,function() {
          if(currentdirindex === 0) {
            renderMacroList(Macros.macros);
          ...
          window.parent.success($.multilang('macro_import_success'));
        });
      });
    }
  });
...
function delMacroElement(data, guid) {
  ...
  for (var i = 0; i < data.length; i++) {
    if(!data[i].Type) {
      ...
      delMacroElement(data[i].Data, guid);
    }
  }
}
